...
/// Enhanced to support multiple completely separate line paths.
...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UILineRendererList : UIPrimitiveBase
    {
        ...
        // Changed from List<Vector2> to a List of independent point paths
        [SerializeField, Tooltip("A list of separate lines, where each line is represented by a list of points.")]
        internal List<List<Vector2>> m_lines = new List<List<Vector2>>();
        ...
        /// <summary>
        /// Collection of separate lines to be drawn.
        /// </summary>
        ...
        // Helper methods for structural interaction
        public void AddLine(List<Vector2> newLine)
        {
            m_lines.Add(newLine);
            SetAllDirty();
        }
        ...
        private void PopulateMesh(VertexHelper vh, List<List<Vector2>> linesToDraw)
        {
            ...
            // Global collection tracking segments across all lines
            var segments = new List<UIVertex[]>();
            
            // Keeps track of the index boundaries where one independent line finishes and another begins
            var lineBoundaries = new List<int>();

            foreach (var originalPoints in linesToDraw)
            {
                if (originalPoints == null || originalPoints.Count < 2) continue;

                List<Vector2> pointsToDraw = new List<Vector2>(originalPoints);

                // Apply Bezier smoothing per separate line sequence
                if (BezierMode != BezierType.None && BezierMode != BezierType.Catenary && pointsToDraw.Count > 3)
                {
                    ...
                    switch (BezierMode)
                    {
                        case BezierType.Basic: pointsToDraw = bezierPath.GetDrawingPoints0(); break;
                        case BezierType.Improved: pointsToDraw = bezierPath.GetDrawingPoints1(); break;
                        default: pointsToDraw = bezierPath.GetDrawingPoints2(); break;
                    }
                }
                ...
                int startSegmentCount = segments.Count;
                ...
                else
                {
                    for (var i = 1; i < pointsToDraw.Count; i++)
                    {
                        ...
                        segments.Add(CreateLineSegment(start, end, SegmentType.Middle));
                        if (lineCaps && i == pointsToDraw.Count - 1) segments.Add(CreateLineCap(start, end, SegmentType.End));
                    }
                }

                // Record where this specific line's processing ends so we don't accidentally join it to the next independent line
                lineBoundaries.Add(segments.Count);
            }

            int currentBoundaryIndex = 0;

            // Add the line segments to the vertex helper, creating joins ONLY within continuous sub-lines
            for (var i = 0; i < segments.Count; i++)
            {
                bool canJoinWithNext = !lineList && (i < segments.Count - 1);
                
                // If the next segment belongs to a new separate line sequence, suppress connection joins
                if (canJoinWithNext && lineBoundaries.Count > 0 && i == lineBoundaries[currentBoundaryIndex] - 1)
                {
